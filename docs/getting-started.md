...
On launch, host Pi API-key IDs that exist in `sbx` are copied into `sbx secret set <same-id>` through stdin. OAuth providers (`openai-codex`, `xai`, `openrouter`) keep their host tokens in the sandbox `auth.json`. Host catalogs come from `models-store.json`. `qwen-token-plan` has no `sbx` service and stays unmatched. `--docker-sandbox-sync mirror` also copies `npm:`/`git:` package specs and local skills; host-path packages stay on the host. Disable secret sync with `--docker-sandbox-no-host-auth`.
